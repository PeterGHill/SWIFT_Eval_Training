...
filename = r"C:\Users\PeterHill\SWIFT_Eval_Practical\TAMSAT\rfe2018_06_26.v3.nc" # Need to change to match the location of the file on your computer. 
...
end_time = datetime.datetime(2018,6,26,23,30,0,0)
...
# Plot GPM map
plotting_routines.plot_map(g_rain.mean(axis=0) * 24., g_lat, g_lon, cbar_levels=np.arange(0,90,10), savename='GPM_IMERG_rainfall_map_26June2018.png', title='GPM IMERG Rainfall') # Change savename as approrpitate
...
plotting_routines.timeseries_plot(g_rain_kumasi.mean(axis=(1,2)), g_datevar_kumasi, 'Rainfall (mm h1-1)', savename='Kumasi_GPM_diurnal_cycle_26June2018.png')

# regrid both datasets to same resolution
new_res =  # Enter value for new resolution in degrees. Change to look at how differences change with averaging scales
new_lat = np.arange(0.0, 20.0, new_res)
new_lon = np.arange(-15., 15., new_res) # regridding is slow so focus on reduced domain
...
plotting_routines.plot_map(regridded_t_rain, new_lat, new_lon, cbar_levels=np.arange(0,90,10), savename='Regridded_TAMSAT_rainfall_map_26June2018.png') # Change savename as approrpitate
plotting_routines.plot_map(regridded_g_rain * 24., new_lat, new_lon, cbar_levels=np.arange(0,90,10), title='GPM IMERG Rainfall', savename='Regridded_GPM_IMERG_rainfall_map_26June2018.png') # Change savename as approrpitate
difference = regridded_t_rain - (regridded_g_rain * 24.)
plotting_routines.plot_map(difference, new_lat, new_lon, cbar_levels=np.arange(-50,51,10), title='TAMSAT - GPM IMERG', savename='TAMSAT_minus_GPM_rainfall_map_26June2018.png') # Change savename as approrpitate

# Scatterplot comparison, incuding correlations
plotting_routines.scatter_plot(regridded_t_rain, regridded_g_rain * 24., 'TAMSAT rainfall (mm day-1)', 'GPM IMERG Rainfall (mm day-1)', savename='TAMSAT_GPM_scatterplot_26June2018.png')
